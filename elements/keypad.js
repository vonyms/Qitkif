/* eslint-disable react-native/no-inline-styles */
import React, {Component} from 'react';
import {
  Appearance,
  Image,
  StyleSheet,
  Text,
  TouchableOpacity,
  View,
} from 'react-native';
import {colors} from '../helpers/colors';
import Button from './button';

const colorScheme = Appearance.getColorScheme();

export default class Keypad extends Component {
  state = {
    code: '',
    secrets: {
      first: null,
      second: null,
      third: null,
      fourth: null,
    },
  };
  render() {
    return (
<View>
  <View style={_styles.wrapper}>
    <Text style={_styles.text_content} accessible={true} accessibilityLabel="First secret">{this.state.secrets.first}</Text>
    <Text style={_styles.text_content} accessible={true} accessibilityLabel="Second secret">{this.state.secrets.second}</Text>
    <Text style={_styles.text_content} accessible={true} accessibilityLabel="Third secret">{this.state.secrets.third}</Text>
    <Text style={_styles.text_content} accessible={true} accessibilityLabel="Fourth secret">{this.state.secrets.fourth}</Text>
  </View>

  <View style={{marginTop: 20}}>
    <View style={_styles.pad_container}>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.getVal(1)}
        accessible={true}
        accessibilityLabel="Press number 1"
        accessibilityRole="button"
        accessibilityHint="Press to add 1 to the code">
        <Text style={_styles.pad_content}>1</Text>
      </TouchableOpacity>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.getVal(2)}
        accessible={true}
        accessibilityLabel="Press number 2"
        accessibilityRole="button"
        accessibilityHint="Press to add 2 to the code">
        <Text style={_styles.pad_content}>2</Text>
      </TouchableOpacity>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.getVal(3)}
        accessible={true}
        accessibilityLabel="Press number 3"
        accessibilityRole="button"
        accessibilityHint="Press to add 3 to the code">
        <Text style={_styles.pad_content}>3</Text>
      </TouchableOpacity>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.getVal(4)}
        accessible={true}
        accessibilityLabel="Press number 4"
        accessibilityRole="button"
        accessibilityHint="Press to add 4 to the code">
        <Text style={_styles.pad_content}>4</Text>
      </TouchableOpacity>
    </View>
    
    <View style={[_styles.pad_container, {marginTop: 15}]}>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.getVal(5)}
        accessible={true}
        accessibilityLabel="Press number 5"
        accessibilityRole="button"
        accessibilityHint="Press to add 5 to the code">
        <Text style={_styles.pad_content}>5</Text>
      </TouchableOpacity>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.getVal(6)}
        accessible={true}
        accessibilityLabel="Press number 6"
        accessibilityRole="button"
        accessibilityHint="Press to add 6 to the code">
        <Text style={_styles.pad_content}>6</Text>
      </TouchableOpacity>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.getVal(7)}
        accessible={true}
        accessibilityLabel="Press number 7"
        accessibilityRole="button"
        accessibilityHint="Press to add 7 to the code">
        <Text style={_styles.pad_content}>7</Text>
      </TouchableOpacity>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.getVal(8)}
        accessible={true}
        accessibilityLabel="Press number 8"
        accessibilityRole="button"
        accessibilityHint="Press to add 8 to the code">
        <Text style={_styles.pad_content}>8</Text>
      </TouchableOpacity>
    </View>

    <View style={[_styles.pad_container, {marginTop: 15}]}>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.getVal(9)}
        accessible={true}
        accessibilityLabel="Press number 9"
        accessibilityRole="button"
        accessibilityHint="Press to add 9 to the code">
        <Text style={_styles.pad_content}>9</Text>
      </TouchableOpacity>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.getVal(0)}
        accessible={true}
        accessibilityLabel="Press number 0"
        accessibilityRole="button"
        accessibilityHint="Press to add 0 to the code">
        <Text style={_styles.pad_content}>0</Text>
      </TouchableOpacity>
      <TouchableOpacity
        activeOpacity={0.4}
        style={_styles.pad_item}
        onPress={() => this.clearVal()}
        accessible={true}
        accessibilityLabel="Clear the code"
        accessibilityRole="button"
        accessibilityHint="Press to clear the code">
              <Image
                style={_styles.pad_clear}
                source={{
                  uri: 'data:image/png;base64,',
                }}
                />
                </TouchableOpacity>
              </View>
            </View>
          
            <Button
              title="Envoyer"
              bg={colors.primary}
              style={{marginTop: 20}}
              onPress={() => this.props.onSubmit(this.state.code, () => this.reset())}
              loading={this.props.loading}
              accessible={true}
              accessibilityLabel="Submit the code"
              accessibilityRole="button"
              accessibilityHint="Press to submit the entered code"
            />
          </View>
    );
  }
  getVal(value) {
    let newVal = this.state.code;
    if (newVal.length < 4) {
      newVal = this.state.code.concat(value);
      this.setState({code: newVal});
      this.setSecretValue(newVal);
    }
  }
  clearVal() {
    let newVal = this.state.code;
    if (newVal.length > 0) {
      newVal = newVal.slice(0, -1);
      this.setState({code: newVal});
      this.clearSecretValue(newVal);
    }
  }
  clearSecretValue(value) {
    const _secrets = this.state.secrets;
    switch (value.length) {
      case 0:
        _secrets.first = null;
        break;
      case 1:
        _secrets.second = null;
        break;
      case 2:
        _secrets.third = null;
        break;
      case 3:
        _secrets.fourth = null;
        break;
      default:
        break;
    }
    this.setState({secrets: _secrets});
  }
  setSecretValue(value) {
    const _secrets = this.state.secrets;
    switch (value.length) {
      case 1:
        _secrets.first = '*';
        break;
      case 2:
        _secrets.second = '*';
        break;
      case 3:
        _secrets.third = '*';
        break;
      case 4:
        _secrets.fourth = '*';
        break;
      default:
        break;
    }
    this.setState({secrets: _secrets});
  }
  reset() {
    this.setState({
      code: '',
      secrets: {
        first: null,
        second: null,
        third: null,
        fourth: null,
      },
    });
  }
}

const _styles = StyleSheet.create({
  wrapper: {
    flexDirection: 'row',
    justifyContent: 'center',
  },
  text_content: {
    width: 40,
    height: 40,
    borderWidth: 1,
    borderColor: colors.gray, //colorScheme === 'dark' ? colors.light : colors.dark,
    textAlign: 'center',
    textAlignVertical: 'center',
    fontSize: 30,
    color: colorScheme === 'dark' ? colors.light : colors.dark,
    marginRight: 10,
  },
  pad_container: {
    flexDirection: 'row',
    justifyContent: 'center',
  },
  pad_item: {
    width: 50,
    height: 50,
    borderWidth: 0.5,
    borderColor: colors.gray, //colorScheme === 'dark' ? colors.light : colors.dark,
    marginRight: 10,
    justifyContent: 'center',
    alignItems: 'center',
  },
  pad_content: {
    fontSize: 34,
    color: colorScheme === 'dark' ? colors.light : colors.dark,
  },
  pad_clear: {
    width: 30,
    height: 30,
  },
});
